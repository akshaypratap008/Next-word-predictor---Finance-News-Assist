from fastapi import FastAPI
from pydantic import BaseModel
from tensorflow.keras.models import load_model
from tensorflow.keras.preprocessing.text import tokenizer_from_json
from tensorflow.keras.preprocessing.sequence import pad_sequences
import json
import numpy as np

app = FastAPI()

MODEL_VERSION = '1.0.0'

# load model and tokenizer
with open('artifacts/tokenizer_v1.json') as file:
    tokenizer_data = json.load(file)

tokenizer = tokenizer_from_json(tokenizer_data)
lstm_model = load_model('artifacts/model_v1.keras')

model = lstm_model

class PredictRequest(BaseModel):
    user_input : str
    k : int

# home end point
@app.get('/')
def home():
    return {
        'message': 'lstm model api',
        'details': 'api predicts the next word suggestions'
        }

# about end point
@app.get('/about')
def about():
    return {
        'message': 'this api predicts the next word using lstm model'
    }

# prediction end point
@app.post('/predict')
def make_prediction(req: PredictRequest):
    user_input = req.user_input
    k = req.k

    suggestions = []

    tokenized_text = tokenizer.texts_to_sequences([user_input])[0]
    padded_token_text = pad_sequences([tokenized_text], maxlen = 29, padding = 'pre')

    top_k_indices  = np.argsort(lstm_model.predict([padded_token_text])[0])[-k:][::-1]

    for i in top_k_indices:
        for word, index in tokenizer.word_index.items():
            if index == i:
                suggestions.append(word)
    
    return {'suggestions': suggestions}

# health check point
@app.get('/health')
def health_check():
    return {
        'status': 'OK',
        'version': MODEL_VERSION,
        'model_loaded': model is not None
    }

